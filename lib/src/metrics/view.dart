  ...
  bool matches(String instrumentName, APIInstrument instrument) {
    ...
    if (instrumentType != null) {
      if (instrumentType == APICounter && !_isCounter(instrument)) {
        return false;
      } else if (instrumentType == APIUpDownCounter && !_isUpDownCounter(instrument)) {
        return false;
      } else if (instrumentType == APIHistogram && !_isHistogram(instrument)) {
        return false;
      } else if (instrumentType == APIGauge && !_isGauge(instrument)) {
        return false;
      } else if (instrumentType == APIObservableCounter && !_isObservableCounter(instrument)) {
        return false;
      } else if (instrumentType == APIObservableUpDownCounter && !_isObservableUpDownCounter(instrument)) {
        return false;
      } else if (instrumentType == APIObservableGauge && !_isObservableGauge(instrument)) {
        return false;
      }
    }
