...
class OtlpHttpSpanExporter implements SpanExporter {
  ...
  Future<void> _tryExport(List<Span> spans) async {
    ...
    if (OTelLog.isDebug()) {
      OTelLog.debug('OtlpHttpSpanExporter: Preparing to export ${spans.length} spans');
      for (var span in spans) {
        OTelLog.debug('  Span: ${span.name}, spanId: ${span.spanContext.spanId}, traceId: ${span.spanContext.traceId}');
      }
    }

    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Transforming ${spans.length} spans');
    final request = OtlpSpanTransformer.transformSpans(spans);
    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Successfully transformed spans');
    ...
    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Sending export request to $endpointUrl');
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Export request completed successfully');
      } else {
        final String errorMessage = 'OtlpHttpSpanExporter: Export request failed with status code ${response.statusCode}';
  ...
  Future<void> export(List<Span> spans) async {
    ...
    if (spans.isEmpty) {
      if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: No spans to export');
      return;
    }

    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Beginning export of ${spans.length} spans');
    ...
    try {
      await exportFuture;
      if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Export completed successfully');
    } catch (e) {
      if (_isShutdown && e is StateError && e.message.contains('shut down during')) {
        ...
        if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Export was interrupted by shutdown, suppressing error');
      } else {
  ...
  Future<void> _export(List<Span> spans) async {
    ...
    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Attempting to export ${spans.length} spans to ${_config.endpoint}');
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (wasShutdownDuringRetry && attempts > 0) {
          if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Export interrupted by shutdown');
          throw StateError('Exporter was shut down during export');
        }

        await _tryExport(spans);
        if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Successfully exported spans');
        return;
      } on http.ClientException catch (e, stackTrace) {
        if (OTelLog.isError()) OTelLog.error('OtlpHttpSpanExporter: HTTP error during export: $e');
        ...
        if (wasShutdownDuringRetry) {
          if (OTelLog.isError()) OTelLog.error('OtlpHttpSpanExporter: Export interrupted by shutdown');
          throw StateError('Exporter was shut down during export');
        }
        ...
        if (!shouldRetry) {
          if (OTelLog.isError()) OTelLog.error('OtlpHttpSpanExporter: Non-retryable HTTP error, stopping retry attempts');
          rethrow;
        }

        if (attempts >= maxAttempts - 1) {
          if (OTelLog.isError()) OTelLog.error('OtlpHttpSpanExporter: Max attempts reached ($attempts out of $maxAttempts), giving up');
          rethrow;
        }

        final delay = _calculateJitteredDelay(attempts);
        if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Retrying export after ${delay.inMilliseconds}ms...');
        await Future<void>.delayed(delay);
        attempts++;
      } catch (e, stackTrace) {
        if (OTelLog.isError()) OTelLog.error('OtlpHttpSpanExporter: Unexpected error during export: $e');
        if (OTelLog.isError()) OTelLog.error('Stack trace: $stackTrace');
  ...
  Future<void> forceFlush() async {
    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Force flush requested');
    if (_isShutdown) {
      if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Exporter is already shut down, nothing to flush');
      return;
    }

    // Wait for any pending export operations to complete
    if (_pendingExports.isNotEmpty) {
      if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Waiting for ${_pendingExports.length} pending exports to complete');
      try {
        await Future.wait(_pendingExports);
        if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: All pending exports completed');
      } catch (e) {
        if (OTelLog.isError()) OTelLog.error('OtlpHttpSpanExporter: Error during force flush: $e');
      }
    } else {
      if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: No pending exports to flush');
    }
  }

  @override
  Future<void> shutdown() async {
    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Shutdown requested');
    if (_isShutdown) {
      return;
    }
    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Shutting down - waiting for ${_pendingExports.length} pending exports');
    ...
    if (pendingExportsCopy.isNotEmpty) {
      if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Waiting for ${pendingExportsCopy.length} pending exports with timeout');
      try {
        ...
        await Future.wait(pendingExportsCopy).timeout(const Duration(seconds: 10), onTimeout: () {
          if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Timeout waiting for exports to complete');
          return Future.value([]);
        });
      } catch (e) {
        if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Error during shutdown while waiting for exports: $e');
      }
    }

    if (OTelLog.isDebug()) OTelLog.debug('OtlpHttpSpanExporter: Shutdown complete');
  }
}
