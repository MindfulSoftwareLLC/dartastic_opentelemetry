...
class OtlpSpanTransformer {
  ...
  static proto.ExportTraceServiceRequest transformSpans(List<Span> spans) {
    var exportTraceServiceRequest = proto.ExportTraceServiceRequest();
  ...
  static String _getResourceServiceName(Attributes attributes) {
    for (final attr in attributes.toList()) {
      if (attr.key == 'service.name' && attr.value != null) {
  ...
  static proto.AnyValue _transformAttributeValue(
    ...
    if (attr.value is String) {
      anyValue.stringValue = attr.value;
    } else if (attr.value is bool) {
      anyValue.boolValue = attr.value;
    } else if (attr.value is int) {
      anyValue.intValue = Int64(attr.value);
    } else if (attr.value is double) {
      anyValue.doubleValue = attr.value;
