...
```bash
chmod +x tool/update_coverage.sh
...
1. **Avoid Shared Resources**: Use unique ports, file paths, and identifiers in tests.
2. **Manage Resource Cleanup**: Ensure resources are properly cleaned up in `tearDown`.
3. **Add Timeout Management**: Consider using explicit timeouts in tests.
4. **Use Fallback Mechanisms**: Provide fallback data when tests might fail in certain environments.
...
You can set these in your own scripts as needed.
