...
- **Breaking:** `Tracer.startSpan` no longer auto-activates the returned span (matching the new API contract and the OpenTelemetry specification). Use `Tracer.withSpan` / `Tracer.withSpanAsync` (or the `recordSpan` / `recordSpanAsync` / `startActiveSpan*` convenience methods) to make a span active for a scope.
...
- `Tracer.recordSpan` / `recordSpanAsync` now activate the started span via `withSpan` / `withSpanAsync` for the duration of `fn`. Inside `fn`, `tracer.currentSpan` correctly returns the started span (was silently broken under 1.0.0-beta of the API).
