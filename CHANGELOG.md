...
### Changed
- **Calling `OTel.tracer()` / `OTel.tracerProvider()` / `OTel.meterProvider()` / `OTel.loggerProvider()` (and the `add*Provider` variants) before `OTel.initialize` no longer throws a cast `TypeError`** — they return SDK-typed providers that wrap the API's lazy noop factory and behave per spec ("in the absence of an installed SDK, the Trace API is a 'no-op' API"). Makes library code that captures a tracer at module-load time (`final _tracer = OTel.tracer('my-lib');`) safe without `late final` workarounds.
- **Late-binding proxies for `OTel.tracerProvider()`, `OTel.tracer()`, `OTel.loggerProvider()`, `OTel.logger()` (and named variants).** These now return cached proxy objects (`LateBindingTracerProvider` / `LateBindingTracer` / `LateBindingLoggerProvider` / `LateBindingLogger`) whose identity is stable across `OTel.initialize`. Every method on a proxy re-resolves the *current* underlying SDK provider on each call — pre-init that's a noop SDK wrapper around the API noop factory; post-init it's the real SDK provider. Library code that captures a tracer at module-load time (the canonical Genkit pattern: `final _tracer = OTel.tracer();` at the top of a `.dart` file) now transparently starts producing real, recorded spans the moment the host application calls `OTel.initialize`. Matches Java's `GlobalOpenTelemetry.get()` / `getTracer()` semantics. Implementation lives in `lib/src/trace/late_binding_tracer.dart` and `lib/src/logs/late_binding_logger.dart`; the file-level docstrings explain the design and the rationale for choosing proxies over a mutable-delegate swap.
  - `OTel.meterProvider()` follows the same pattern (`LateBindingMeterProvider` in `lib/src/metrics/late_binding_meter.dart`), but **`OTel.meter()` opts out** of meter-level late binding and returns the real SDK [Meter] directly. The reason: concrete instruments (counters / histograms / gauges) hold an `instrument.meter` back-reference that callers and tests routinely compare for identity. A proxy meter would break those checks, and the cost-benefit of meter-level late binding doesn't pencil out (metrics module-load capture is rare; tracer module-load capture — Genkit-style — is the load-bearing case).
  - Span identity from a captured pre-init tracer: a span returned by `tracer.startSpan(...)` is still a concrete one-shot object created against whatever underlying tracer was resolved at that moment. So spans created pre-init remain noop spans even after a later `OTel.initialize` call — that's the spec-correct outcome ("you can't retroactively make a span recorded"). Late binding only affects future `startSpan` calls on the proxy.
- **`OTel.initialize` now tracks user-initialization separately from "is a factory installed".** Previously the second-call guard was `OTelFactory.otelFactory != null` — but the API layer auto-installs a lazy noop factory on first access, so `OTel.initialize` would throw on the legitimate first user call if anything had touched the API beforehand. Now guarded by a `_userInitialized` boolean. First call upgrades the noop to the SDK factory (Java/JS/Python `setGlobal*` pattern); second still throws.
- `OTel._getAndCacheOtelFactory()` no longer hard-casts to `OTelSDKFactory`. The cached field is now typed as the base `OTelFactory` so it can hold either the SDK factory or the API noop. SDK-specific factory methods (e.g. `OTel.resource(...)`) keep their per-call-site cast. `OTel.initialize` now refreshes the local SDK factory cache after installing the real factory, so SDK-specific casts hit the freshly-installed factory rather than the stale API noop that any pre-init access may have cached.
- OTel default values (resource, sampler, timeProvider) are now applied to each underlying SDK provider **exactly once** per provider instance, tracked via identity sets. The pre-existing per-access `??=` re-application would silently overwrite user mutations made between calls (e.g. `provider.resource = null` immediately followed by `provider.ensureResourceIsSet()`); the once-per-instance application preserves the original "first call wins, then user controls" semantics that the existing `ensureResourceIsSet` tests depend on.
- `OTel.reset` now also clears the user-init flag and the cached noop SDK wrappers, so test fixtures can `reset` → `initialize` cleanly.
- Bumped `dartastic_opentelemetry_api` to `^1.0.0-beta.6`.

### Added
- `OTel.isInitialized` static getter — `true` if `initialize` has been called explicitly, `false` if only the auto-installed noop default is in place.
- `LateBindingTracerProvider`, `LateBindingTracer`, `LateBindingMeterProvider`, `LateBindingLoggerProvider`, `LateBindingLogger` — public SDK types so callers can `is`-check or cast if they need to, e.g. when bridging from older Workiva-shaped APIs that exposed a global-proxy pattern.
- Internal-only static methods on `OTel`: `internalResolveRealTracerProvider(name)`, `internalGetCachedLateBindingTracer(...)`, `internalResolveRealMeterProvider()`, `internalResolveRealLoggerProvider()`, `internalGetCachedLateBindingLogger(...)`. Marked `@internal`; used by the proxy classes to forward calls to the current real underlying provider.

### Fixed
- `test/unit/shutdown_does_not_hang_test.dart` subprocess pubspec now includes a `dependency_overrides` for the API path. Without it, the subprocess could not resolve the SDK's `dartastic_opentelemetry_api: ^1.0.0-beta.6` constraint against the in-flight local API (`1.0.0-beta.6-wip`), because pub only honors `dependency_overrides` from the root package — not from a transitive dep like the SDK.
