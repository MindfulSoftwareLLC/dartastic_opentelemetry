...
- **Breaking:** `Tracer.withSpan` / `Tracer.withSpanAsync` parameter type narrowed to `covariant Span` (the SDK class) from `APISpan`. SDK code should not deal in `APISpan` — `startSpan` returns `Span`, and the runtime `if (span is Span)` guards inside `withSpan` are now compile-time enforced.
- Added `OTel.withSpan(span, fn)` and `OTel.withSpanAsync(span, fn)` static convenience methods that delegate to the default tracer — saves callers from threading a `Tracer` reference for the common activation case.
- `Tracer.startActiveSpan` / `Async` now declare their `fn` parameter as `T Function(Span span)` (was `APISpan span`), again to keep SDK code in SDK types.
